...
impl ReplayStage {
    ...
    fn handle_votable_banks(
        ...
        // Use the last bank for commitment cache and voting
        if let Some(last_bank) = banks.last() {
            ...
            Self::update_commitment_cache(
                ...
                progress.get_fork_stats(last_bank.slot()).unwrap().total_stake,
                node_vote_state,
                lockouts_sender,
            );
    ...
    fn first_mostly_confirmed_bank(
        ...
        let mut current_bank = Some(bank);
        while let Some(bank) = current_bank {
            if let Some(fork_stats) = progress.get_fork_stats(bank.slot()) {
                if fork_stats.is_mostly_confirmed {
                    return Some(bank);
                }
            }
    ...
    fn banks_between_and_including(
        start_slot: Slot,
        end_bank: Arc<Bank>,
    ) -> Vec<Arc<Bank>> {
        let mut banks = vec![];
        let mut current_bank = Some(end_bank);
        
        while let Some(bank) = current_bank {
            if bank.slot() >= start_slot {
    ...
    fn tower_duplicate_confirmed_forks(
        ...
        (duplicate_confirmed_forks, mostly_confirmed_slots)
    }
...
pub(crate) mod tests {
    ...
    fn test_compute_bank_stats_confirmed() {
        ...
        // bank 1, so no slot should be confirmed.
        {
            ...
            let (confirmed_forks, _mostly_confirmed_slots) = ReplayStage::tower_duplicate_confirmed_forks(
                &tower,
